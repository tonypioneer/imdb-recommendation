...
class SVD_SGD:
    ...
    def fit(self, train_data):
        ...
        self.user_map = {user: i for i, user in
                         enumerate(train_data['userId'].unique())}
        self.item_map = {item: i for i, item in
        ...
        for epoch in range(self.n_epochs):
            for row in train_data.itertuples():
                ...
                self.user_factors[user_idx] += self.lr * (
                            error * self.item_factors[item_idx] - self.reg *
                            self.user_factors[user_idx])
                self.item_factors[item_idx] += self.lr * (
                            error * self.user_factors[user_idx] - self.reg *
                            self.item_factors[item_idx])

    def predict(self, user, item):
        if user >= len(self.user_factors) or item >= len(self.item_factors):
            return np.mean(
                train_data['rating'])  # Return global mean if out of bounds
    ...
    def fit(self, train_data):
        ...
        for epoch in range(self.n_epochs):
            for row in train_data.itertuples():
                ...
                update_user = self.lr * (
                            error * self.item_factors[item_idx] - self.reg *
                            self.user_factors[user_idx]) + noise
                update_item = self.lr * (
                            error * self.user_factors[user_idx] - self.reg *
                            self.item_factors[item_idx]) + noise
    ...
    def fit(self, train_data):
        ...
        for epoch in range(self.n_epochs):
            for row in train_data.itertuples():
                ...
                update_user = self.lr * (
                            error * self.item_factors[item_idx] - self.reg *
                            self.user_factors[user_idx]) - self.friction * \
                              self.user_factors[user_idx] + noise
                update_item = self.lr * (
                            error * self.user_factors[user_idx] - self.reg *
                            self.item_factors[item_idx]) - self.friction * \
                              self.item_factors[item_idx] + noise
                self.user_factors[user_idx] += np.nan_to_num(update_user)
                self.item_factors[item_idx] += np.nan_to_num(update_item)
...
def evaluate_model(model, test_data):
    ...
    for row in test_data.itertuples():
        ...
        if user == -1 or item == -1:
            predictions.append(
                np.mean(train_data['rating']))  # Fallback to global mean rating
        else:
...
svd_sgld = SVD_SGLD(n_factors=20, lr=0.005, reg=0.05, n_epochs=20,
                    noise_scale=0.01)
...
svd_sghmc = SVD_SGHMC(n_factors=20, lr=0.005, reg=0.05, n_epochs=20,
                      friction=0.05, noise_scale=0.01)
...
class HybridRecommender:
    ...
    def recommend(self, user, movie_ids):
        ...
        movie_indices = [self.svd_model.item_map.get(movie_id) for movie_id in
                         movie_ids if movie_id in self.svd_model.item_map]
        ...
        for i in range(len(movie_indices)):
            svd_scores = [self.svd_model.predict(user, list(
                self.svd_model.item_map.keys())[idx]) for idx in knn_indices[i]]
            recommendations.extend(
                zip([list(self.svd_model.item_map.keys())[idx] for idx in
                     knn_indices[i]], svd_scores))
...
# Use the HybridRecommender with the SGD optimizer to make recommendations and
...
user_id = 1
movie_ids = test_data['movieId'].unique()[
            :100]  # Use a subset of movie IDs for demonstration
